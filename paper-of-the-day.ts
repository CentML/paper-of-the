...
async function alignedWithCentML(text: string): Promise<boolean> {
    ...
    const response = await client.chat.completions.create({
        messages: [
            {
                role: "system",
                content: `You are a helpful AI assistant. Here's the json schema you need to adhere to: <schema>${JSON.stringify(
                    schema
                )}</schema>`,
            },
            {
    ...
    console.log(
        "alignedWithCentML tokens per second:",
        completionTokens / (completionTime / 1000)
    )
...
async function mostImpactful(paper1: Paper, paper2: Paper): Promise<Paper> {
    ...
    const messages: Array<OpenAI.ChatCompletionMessageParam> = [
        {
            role: "system",
            content: `You are an expert at comparing academic papers while following instructions. Here's the json schema you need to adhere to in your response: <schema>${JSON.stringify(
                schema
            )}</schema>`,
        },
        {
    ...
    // the latency of the request and time to first token are big factors
    console.log(
        "mostImpactful tokens per second:",
        completionTokens / (completionTime / 1000)
    )
...
async function summarize(
    ...
    const response = await client.chat.completions.create({
        messages: [
            {
                role: "system",
                content: `You are an expert at summarizing information for mass consumption as a post. Here's the JSON schema you must adhere to: <schema>${JSON.stringify(
                    schema
                )}</schema>`,
            },
            {
                role: "user",
                content: `Create an approximately ${characters} character summary of the following paper. ${summaryPrompt} Arxid Id: ${
                    paper.arxivId
                } Paper:\n${await paper.text()}`,
            },
        ],
...
Deno.cron("paper of the day", "1 9 * * *", async () => {
    ...
    try {
        const today = new Date().toISOString().split("T")[0] // Get UTC date in YYYY-MM-DD format
        const lastSuccess = await kv.get(["last_success_date"])

        if (lastSuccess.value === today) {
            console.log("Workflow already completed today. Skipping.")
            return
        }
        ...
        // Only update if workflow was successful
        await kv.set(["last_success_date"], today)
    } catch (error) {
